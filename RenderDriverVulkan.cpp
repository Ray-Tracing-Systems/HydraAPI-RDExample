...
void RD_Vulkan::prepareCommandBuffers(uint32_t current_image) {
  ...
  const uint32_t directLightsCount = directLights.size();
  const uint32_t spotLightsCount = spotLights.size();
  vkCmdPushConstants(commandBuffers[current_image], resolvePipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(directLightsCount), &directLightsCount);
  vkCmdPushConstants(commandBuffers[current_image], resolvePipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(directLightsCount), sizeof(spotLightsCount), &spotLightsCount);
...
void RD_Vulkan::createPipelines() {
  ...
  resolveConfig.pushConstants.emplace_back(VkPushConstantRange{ VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(uint2) });
...
void RD_Vulkan::updateUniformBuffer(uint32_t current_image) {
  ...
  for (const auto& light : directLights) {
    float4x4 pr = orthographic_projection_matrix(-light.outerRadius, light.outerRadius, -light.outerRadius, light.outerRadius, 0.1, 200.0);
...
void RD_Vulkan::InstanceLights(int32_t a_light_id, const float* a_matrix, pugi::xml_node* a_custAttrArray, int32_t a_instNum, int32_t a_lightGroupId)
{
  ...
  if (directLightLib.count(lightId)) {
    for (uint32_t i = 0; i < a_instNum && directLights.size() < MAX_DIRECT_LIGHTS; ++i) {
